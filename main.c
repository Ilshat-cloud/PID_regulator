...
void main_func(void *argument)
{
  ...
  for(;;)
  {
    ...
    case 5:
      HAL_ADC_Start_DMA(&hadc1,(uint32_t*)&dma,2); 
      osDelay(10);
      t_cur[0]=(dma[0]+t_cur[0])/2;
      NTC_t=0;
      while (NTC_10k[NTC_t]<t_cur[0])
        {
          NTC_t++;
        }
      NTC_t=(NTC_t==0)?1:NTC_t;
      if ((t_cur[0]-NTC_10k[NTC_t-1])>(NTC_10k[NTC_t]-t_cur[0]))
        {
          Temp_ADC=NTC_t-5;
        }else
        {
   ...
   //button=TIM4->CCR2-TIM4->CCR1; 
    ...
    if (start)
    {
      ...
      if (mistake)
      {
        ...
        {
          if (direct==0){HAL_GPIO_WritePin(RELAY_HIST_GPIO_Port,RELAY_HIST_Pin, GPIO_PIN_RESET);
          }else{HAL_GPIO_WritePin(RELAY_HIST_GPIO_Port,RELAY_HIST_Pin, GPIO_PIN_SET);}
        }
        else if (Temp_ADC<=target1-hysteresys)
        {
          if (direct==0){HAL_GPIO_WritePin(RELAY_HIST_GPIO_Port,RELAY_HIST_Pin, GPIO_PIN_SET);
          }else{HAL_GPIO_WritePin(RELAY_HIST_GPIO_Port,RELAY_HIST_Pin, GPIO_PIN_RESET);}
        }
      ...
      if ((point_num>0)&&(worktime>=(point[curPoint].Minuts*60+point[curPoint].Sec)))
        {
          ...
          if (curPoint<=point_num){
            if (curPoint%2==0)
            {
              ...
              Temp_start=point[curPoint-1].target;
  ...
  for(;;)
  {
    switch(flag)
    {
      ...
      case 1:
        ...
        SendByte(0xEF,1);  //0
        PrintStr("C");
        if (worktime_old>worktime+10){worktime_old=0;}
